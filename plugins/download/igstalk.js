...
async function igstalk(Username) {
  try {
    const trResponse = await axios.get(
      `https://tr.deployers.repl.co/igstalk?user=${Username}`,
    );
    ...
    try {
      const dumpoirResponse = await axios.get(
        `https://dumpoir.com/v/${Username}`,
        {
          headers: {
            "user-agent":
      ...
      const result = {
        bio: $("#user-page > div.user > div > div.col-md-5.my-3 > div")
          .text()
          .trim(),
