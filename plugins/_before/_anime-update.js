...
export async function before(m) {
  ...
  if (chat && chat.updateAnime) {
    chat.updateAnimeInterval = setTimeout(
      async () => {
        try {
          ...
          if (chat.lastAnime.includes(title))
          ...
          if (chat.lastAnime.indexOf(length) !== -1)
            chat.lastAnime.splice(chat.lastAnime.indexOf(length), 1);
